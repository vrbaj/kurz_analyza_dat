...
seznam_clf = [("svm", SVC()), # SVM model
              ("tree", DecisionTreeClassifier()), # Rozhodovací strom
              ("rf", RandomForestClassifier())] # Náhodný les
...
# Iterace skrz jednotlivé klasifikátory a jejich nastavení parametrů
for klasifikator, nastaveni_parametru in zip(seznam_clf, seznam_nastaveni_parametru):
    # Vytvoření pipeline pro zvolený klasifikátor
    pipeline = Pipeline([
        ('scaler', StandardScaler()), # Škálování dat různých rozsahů na stejný interval
        klasifikator # Zvolený klasifikátor
    ])
...
# }
    ...
    grid_search = GridSearchCV(pipeline, nastaveni_parametru, cv=5, scoring='accuracy')
    # Trénování dat pro všechny kombinace hyperparametrů
    grid_search.fit(X_train, y_train)

    # Vypsání nejlepších hyperparametrů pro zvolený klasifikátor

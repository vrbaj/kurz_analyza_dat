...
def get_CI() -> ForecastResult:
    ...
    for n in range(n_bootstrap):
        train_res = resample(train)
        model = fit_xgboost(train_res, feature_cols)
    ...
    predikce = np.mean(preds)

    lower = np.percentile(preds, 25)
    upper = np.percentile(preds, 75)
